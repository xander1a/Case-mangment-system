@extends('layouts.app')
@section('title', 'Add Psychological Support')
@section('content')
    <div class="max-w-4xl mx-auto mt-10 bg-white p-6 rounded shadow">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold">Add New Psychological Support</h2>
        ...
        <form action="{{ route('psychological-supports.store') }}" method="POST">
            @csrf
    ...
    <select id="case_id" 
        ...
        @endforeach
            ...
            <div>
    <label for="counselor_id" class="block text-sm font-medium text-gray-700 mb-2">Counselor *</label>
    <select id="counselor_id" 
        ...
        @foreach($counselors as $counselor)
            <option value="{{ $counselor->id }}" 
                {{ old('counselor_id', auth()->user()->role === 'counselor' ? auth()->id() : '') == $counselor->id ? 'selected' : '' }}>
                {{ $counselor->name }}
            </option>
        @endforeach
    </select>
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route('psychological-supports.index') }}" 
