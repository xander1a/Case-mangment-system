...
<div>
    <label for="legal_officer_id" class="block text-sm font-medium text-gray-700 mb-2">Legal Officer *</label>
    <select id="legal_officer_id" 
            name="legal_officer_id" 
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            required>
        <option value="">-- Select Legal Officer --</option>
        @foreach($legal_officers as $officer)
            <option value="{{ $officer->id }}" 
                {{ old('legal_officer_id', auth()->user()->role === 'legal_officer' ? auth()->id() : '') == $officer->id ? 'selected' : '' }}>
                {{ $officer->name }}
            </option>
        @endforeach
    </select>
