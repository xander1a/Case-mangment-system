...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8 px-4">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="text-center mb-10">
            <h1 class="text-4xl font-bold text-slate-800 mb-3">Edit Victim Information</h1>
            <p class="text-lg text-slate-600">Update the victim's information below</p>
        ...
        <div class="bg-white border-2 border-slate-200 shadow-2xl">
            ...
            <div class="p-8 lg:p-12">
                <form method="POST" action="{{ route('victims.update', $victim) }}" class="space-y-10">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="border-2 border-slate-100 bg-slate-50/50">
                        ...
                        <div class="p-6 space-y-6">
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                <!-- Violence Type Field -->
                                <div>
                                    <label for="violence_type" class="block text-sm font-bold text-slate-700 mb-3">Violence Type</label>
                                    <div class="relative">
                                        <select name="violence_type" 
                                                id="violence_type" 
                                                class="w-full px-4 py-4 border-2 border-slate-300 bg-white focus:border-blue-500 focus:ring-0 transition-all duration-300 text-slate-700 font-medium appearance-none @error('violence_type') border-red-500 bg-red-50 @enderror">
                                            <option value="" disabled {{ old('violence_type', $victim->violence_type) ? '' : 'selected' }}>Select violence type...</option>
                                            <option value="sexual" {{ old('violence_type', $victim->violence_type) == 'sexual' ? 'selected' : '' }}>Sexual violence</option>
                                            <option value="emotional" {{ old('violence_type', $victim->violence_type) == 'emotional' ? 'selected' : '' }}>Emotional/psychological abuse</option>
                                            <option value="physical" {{ old('violence_type', $victim->violence_type) == 'physical' ? 'selected' : '' }}>Physical violence (beating, injury, etc.)</option>
                                            <option value="economic" {{ old('violence_type', $victim->violence_type) == 'economic' ? 'selected' : '' }}>Economic/financial abuse</option>
                                        </select>
                                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                            <svg class="h-5 w-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <textarea name="address" 
                                          ...
                                          placeholder="Enter complete address...">{{ old('address', $victim->address) }}</textarea>
                                @error('address')
                                    <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="other_details" 
                                          ...
                                          placeholder="Additional information, notes, or special circumstances...">{{ old('other_details', $victim->other_details) }}</textarea>
                                @error('other_details')
                                    <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
